...
describe("GET /hotels/:hotelId/vacancies", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 200 and hotel with rooms and booking count", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
      ...
      const response = await server.get(`/hotels/${createdHotel.id}/vacancies`).set("Authorization", `Bearer ${token}`);
